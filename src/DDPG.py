...
if __name__ == '__main__':
   ...
   log_path = os.path.join("Modelos", "DDPG", "DDPG-01")
   model = DDPG('MlpPolicy', env, verbose=1, device='cuda', tensorboard_log=log_path) #, learning_rate=0.00003
   #model = DDPG.load("./Modelos/DDPG/DDPG-01/model_640000_steps", env=env)
   eval_callback = EvalCallback(env, best_model_save_path=log_path,
                                 log_path=log_path, eval_freq=5000,
                                 deterministic=True, render=False)
   checkpoint_callback = CheckpointCallback(save_freq=10000, save_path='./Modelos/DDPG/DDPG-01',
                                       name_prefix='model')
   ...
   model_path = os.path.join("Modelos", "DDPG", "DDPG-01")
